...
class UVO(data.Dataset):
    r"""A torch Dataset for loading in `the UVO dataset <https://sites.google.com/view/unidentified-video-object/dataset>`_. Will fetch sequences of
    ...
    Args:
        basedir (str): Path to the base directory containing the directories from UVO.
    ...
    Examples::
        ...
        >>> dataset = UVO(
            basedir="UVO/DenseSet/train",
            videos=videos_txt_file,
            return_points=True
            )
    ...
    def __init__(
        ...
        *,
        ...
        self.uvo = YTVOS(self.instance_path)
        ...
        for video in videos:
            ...
            # Identfy video id
            vid_ind, vid_id = find(self.uvo.dataset['videos'], 'file_names', video)

            rgbdir = os.path.join(self.basedir, 'frames/')
            rgb_list = [os.path.join(rgbdir, x) for x in self.uvo.dataset['videos'][vid_ind[0]]['file_names']]

            if self.return_points:
                pointsdir = os.path.join(self.basedir, 'points/')
                points_list = [os.path.join(pointsdir, x.replace('.png','.pkl')) for x in self.uvo.dataset['videos'][vid_ind[0]]['file_names']]
    ...
    def __getitem__(self, idx: int):
        ...
        if self.return_seg:
            ...
            # Annotations info
            anns_ids = self.uvo.getAnnIds(vidIds=seg_info['vid_id'])
            anns = self.uvo.loadAnns(anns_ids)
            ...
            for ind in seg_info['inds']:
                ...
                for a in range(len(anns_ids)):
                    ...
                    try:
                      ann_mask = self.uvo.annToMask(anns[a], ind)
                    except:
                      continue
