...
class YouTubeVIS(data.Dataset):
    r"""A torch Dataset for loading in `the YouTube-VIS dataset <https://youtube-vos.org/dataset/vis/>`_. Will fetch sequences of
    rgb images, instance segmentation labels, SuperPoint features (optional).
    Expects the following folder structure for the YouTube-VIS dataset:
    ...
    .. code-block::
        ...
        ├─ train/
        │  ├─ JPEGImages/
        │  │  ├─ 0a8c467cc3/                        # Video Sequence folder
        │  │  │  ├─ 00000.jpg
        │  │  │  ├─ 00005.jpg
        |  |  |  .
        |  |  |  .
        ...
        |  |  .
        |  |  .
        │  ├─ points/                               # SuperPoint features of Video Sequence frames (Optional)
        │  │  ├─ 0a8c467cc3/
        │  │  │  ├─ 00000.pkl
        │  │  │  ├─ 00005.pkl
        |  |  |  .
        ...
        |  |  .
        |  |  .
        │  ├─ instances.json                        # Instance Segmentations Json file
        ├─ test/
        |  |  .
        |  |  .
    ...
    Args:
        basedir (str): Path to the base train/test directory containing the directories from YouTube-VIS.
        videos (str or tuple of str): Videos to use from sequences (used for creating train/val/test splits). Can
            be path to a `.txt` file where each line is a Video Seqeunce name (`0b1627e896`), a tuple of scene names.
    ...
    Examples::
        ...
        >>> dataset = YouTube-VIS(
            basedir="YouTube-VIS/train",
            videos=("0a8c467cc3", "0b1627e896"),
            return_points=True
            )
        >>> loader = data.DataLoader(dataset=dataset, , collate_fn=vis_custom_collate, batch_size=4)
        >>> images, ann_masks, points, videonames = next(iter(loader))
    ...
    def __init__(
        ...
        *,
        ...
        super(YouTubeVIS, self).__init__()
        ...
        self.ytvis = YTVOS(os.path.join(self.basedir, 'instances.json'))
        ...
        for video in videos:
            ...
            # Identfy video id
            vid_ind, vid_id = find(self.ytvis.dataset['videos'], 'file_names', video)

            rgbdir = os.path.join(self.basedir, 'JPEGImages/')
            rgb_list = [os.path.join(rgbdir, x) for x in self.ytvis.dataset['videos'][vid_ind[0]]['file_names']]

            if self.return_points:
                pointsdir = os.path.join(self.basedir, 'points/')
                points_list = [os.path.join(pointsdir, x.replace('.jpg','.pkl')) for x in self.ytvis.dataset['videos'][vid_ind[0]]['file_names']]
    ...
    def __getitem__(self, idx: int):
        ...
        if self.return_seg:
            ...
            # Annotations info
            anns_ids = self.ytvis.getAnnIds(vidIds=seg_info['vid_id'])
            anns = self.ytvis.loadAnns(anns_ids)
            ...
            for ind in seg_info['inds']:
                ...
                for a in range(len(anns_ids)):
                    ...
                    try:
                      ann_mask = self.ytvis.annToMask(anns[a], ind)
                    except:
                      continue
                    if (ann_mask.shape[0] != self.width) or (ann_mask.shape[1] != self.height):
                        ann_mask = cv2.resize(ann_mask.astype(float), (self.width, self.height), interpolation=cv2.INTER_NEAREST)
