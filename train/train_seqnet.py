...
from model.build_model import build_netvlad, build_seqnet
...
def train(configs):
    ...
    seqlen = seqnet_config['train']['seqlen']
    batch_size = seqnet_config['train']['batch_size']
    epochs = seqnet_config['train']['epochs']
    lr = seqnet_config['train']['lr']
    checkpoint = seqnet_config['train']['checkpoint']
    lambda_d = seqnet_config['train']['lambda_d']
    ## files config
    ...
    # loss
    criterion = DescriptorLoss(seqnet_config)

    # tensorboard writer
    writer = SummaryWriter(log_dir=os.path.join(log_dir, datetime.now().strftime('%b%d_%H-%M-%S')+'_seqnet'))
    ...
    # Optimizer
    optimizer = torch.optim.Adam(seqnet_model.parameters(), lr=lr)
    ...
    for epoch in tqdm(range(epochs), desc='train'):
        ...
        for iteration, batch in enumerate(tqdm(loader)):
            ...
            for obj_key in batch_objects.keys():
                ...
                evol_obj_desc = seqnet_model.module.pool(iter_obj_descs.unsqueeze(0))
            ...
            if sum_iter % checkpoint == 0:
                model_saving_path = os.path.join(save_dir, "seqnet_model_{}.pth".format(sum_iter))
                torch.save(seqnet_model.state_dict(), model_saving_path)
                print("saving model to {}".format(model_saving_path))

    writer.close()

def main():
    parser = argparse.ArgumentParser(description="Training SeqNet")
    parser.add_argument(
        "-c", "--config_file",
        dest = "config_file",
        type = str, 
